...
class add_mean_structure_price: #建物構造ごとの家賃平均を追加。分散、中央値もたす。
    def __init__(self):
        self.means = {}
        self.mean_pad = 120000
        self.stds = {}
...
class area_per_price_predictor:
    ...
    def transform(self,x):
        ex_var = x.drop(["mf_area_sq","mf_areasize"],axis = 1)
        pred = self.model.predict(ex_var)
        hoge = x.assign(avg_cross_pred = x["mf_areasize"].values*np.array(pred))
        # hoge = hoge.assign(avg_pred = pred)
        return hoge
